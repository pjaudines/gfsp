<table class="table datatable" id="outgoingProductsTable" content-editable>
    <thead>
        <tr>
            <th>Outgoing ID</th>
            <th>Product ID</th>
            <th>Name</th>
            <th>Type</th>
            <th>Date</th>
            <th>Sales ID</th>
            <th>Quantity</th>
            <th>Action</th>
        </tr>
    </thead>                                    
        <tbody id="itemsTbody">
<?php
    require '../../php/databaseConn.php';
    $query = $conn->query("select * from `inventory_prodoutgoing`") or die(mysqli_error());
    while($fetch = $query->fetch_array()){
?>
    <tr>
        <td><?php echo $fetch['prodOutgoingID']?></td>
        <td><?php echo $fetch['prodID']?></td>
        <td><?php echo $fetch['prodDesc']?></td>
        <td><?php echo $fetch['prodType']?></td>
        <td><?php echo $fetch['outgoingDate']?></td>
        <td><?php echo $fetch['salesID']?></td>
        <td><?php echo $fetch['itemQty']?></td>
        <td style="padding: 0;">
            <button type="button" class="btn btn-primary btn-rounded btn-icons" data-toggle="modal" data-target="#itemEditStocks"><i class="menu-icon mdi mdi-pencil"></i></button>
        </td>
    </tr>
<?php
    }
    $conn->close();
?>
    </tbody>
</table>