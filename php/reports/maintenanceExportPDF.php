...
class myPDF extends FPDF{
    ...
    function headerTable(){
        ...
        $this->Cell(75,8,'CLient Name',1,0,'C');
        $this->Cell(60,8,'Item Name',1,0,'C');
        $this->Cell(75,8,'Item Type',1,0,'C');
        $this->Cell(40,8,'Maintenance Type',1,0,'C');
    ...
    function viewTable($db){
        $this->SetFont('Times','',12);
        $stmt = $db->query(" select * from `transactions_maintenance-items`,`transactions_maintenance` WHERE `transactions_maintenance-items`.maintenanceID=transactions_maintenance.maintenanceID");
        while($data = $stmt->fetch(PDO::FETCH_OBJ)){
            $this->Cell(0.1);
            $this->Cell(75,8,$data->maintenanceClientName,1,0,'C');
            $this->Cell(60,8,$data->itemName,1,0,'C');
            $this->Cell(75,8,$data->itemType,1,0,'C');
            $this->Cell(40,8,$data->maintenanceType,1,0,'C');
