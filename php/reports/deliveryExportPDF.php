...
class myPDF extends FPDF{
    ...
    function headerTable(){
        ...
        $this->Cell(70);
        $this->Cell(145,8,'Delivery List as of Year 2019',0,0,'C');
        $this->Ln();
        $this->Cell(0.1);
        $this->SetFont('Times','B',10);
        $this->Cell(45,8,'Client Name',1,0,'C');
        $this->Cell(50,8,'Client Address',1,0,'C');
        $this->Cell(50,8,'Item Name',1,0,'C');
        $this->Cell(50,8,'Item Type',1,0,'C');
        $this->Cell(40,8,'Quantity',1,0,'C');
        $this->Cell(45,8,'Creation Date',1,0,'C');
        $this->Ln();

    }
    function viewTable($db){
        $this->SetFont('Times','',12);
        $stmt = $db->query(" select * from `transactions_delivery-items`,`transactions_delivery` WHERE `transactions_delivery-items`.deliveryID=transactions_delivery.deliveryID ");
        while($data = $stmt->fetch(PDO::FETCH_OBJ)){
            $this->Cell(0.1);
            $this->Cell(45,8,$data->deliveredTo,1,0,'C');
            $this->Cell(50,8,$data->deliveryAddress,1,0,'C');
            $this->Cell(50,8,$data->itemName,1,0,'C');
            $this->Cell(50,8,$data->itemType,1,0,'C');
            $this->Cell(40,8,$data->itemQuantity,1,0,'C');
            $this->Cell(45,8,$data->creationDate,1,0,'C');
            $this->Ln();
        }
            $this->Ln(15);
