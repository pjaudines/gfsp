...
<script>
  //DELETE PURCHASE ORDER
    function deletePO(a,b,c,d){
        var purchaseItemID = a;
        var purchaseID = b;
        var itemQuantity = c;
        var prodName = d;
           
        $.ajax({
                url: "../../php/transactions/purchase/deletePurchaseOrderItem.php",
                type: "POST",
                data: {purchaseItemID:purchaseItemID, itemQuantity:itemQuantity, prodName:prodName},
                success: function(result){
                  displayItems(purchaseID);
                }
        });
    }
    ...
    //DISPLAY ITEMS TABLE
    function displayItems(purchaseID){
      $.ajax({
          url: "../../php/transactions/purchase/displayItems.php",
          type: "POST",
    ...
    $(document).ready(function(){
        ...
        $("#btnAddItem").click(function(){
          ...
          if(itmType == "none" || itmName == "none" || itmQty < 1){//start validation
            ...
            $.post("../../php/transactions/purchase/addItems.php",{purchaseID: purchaseID, prodType: itmType, prodDesc: itmName, itmQuantity: itmQty }, function(data, status){
              if(status == "success"){
                displayItems(purchaseID);                
                            ...
                            <div class="row">
                              <div class="col-sm-3">                        
                                <label> Purchase Order No.</label>
                                  <input type="text" class="form-control" disabled value="<?php echo  $_POST['purchaseID']?>" id="txtPurchaseOrderNo">
                              </div>
                            </div>
                            <br>
